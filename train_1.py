...
## Choose a dataset name: Openwebtext or Wikitext
...
if dataset_name == 'wikitext':
    if not os.path.exists(hyp['misc']['data_location']):
        os.makedirs('./data_wikitext', exist_ok=True)
...
if dataset_name == 'openwebtext':
    if not os.path.exists(hyp['misc']['data_location']):
        os.makedirs('./data_openwebtext', exist_ok=True)
        print("downloading data and tokenizing (20 - 30 min)")

        # take 54GB in HuggingFace .cache dir, about 8Mb documents (8,013,769)
        dataset = load_dataset('openwebtext')
        # owt by default only contains the 'train', split, so create a val/test split
        split_dataset = dataset['train'].train_test_split(test_size=0.0005, seed=12345, shuffled=True)
        split_dataset['val'] = split_dataset.pop['test'] # rename the test split above to val

        tokenizer = tiktoken.get_encoding('gpt2')
        raw_tokenized_train = tokenizer.encode_ordinary(dataset['train'])
        raw_tokenized_eval = tokenizer.encode_ordinary(dataset['val'])
